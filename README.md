...
> Current status: secure workspaces, the SQLite-backed dashboard, atomic `.canvasnote` persistence, the infinite canvas, portable media imports, local/embedded video, and timestamp-note seeking are working. Discovery, export, settings, and recovery tools are the next milestone; see [CHANGELOG.md](CHANGELOG.md).

The editor supports notes, checklists, images, file cards, local video, approved YouTube/Vimeo embeds, timestamp notes, native frames and connections, grouping, properties, undo/redo, camera persistence, and trailing 750 ms autosave. Press `N` for a note, `C` for a checklist, `I` to import an image, `Shift+V` to import a video, `F` for a frame, `L` for a connection, and `Ctrl/Cmd+S` to save immediately.
